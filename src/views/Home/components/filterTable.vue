...
export default {
  ...
  computed: {
    tableFilter() {
      return this.$store.state.data.filtered;
    },
    isLoading() {
  ...
  methods: {
      ...
      for (const keys in params) {
        this.tableConfig.columns.forEach(el => {
          if (
            ...
            el.orderBy = "asc";
            this.tableFilter.sort((a, b) => a[keys].localeCompare(b[keys]));
            this.$store.commit("SET_FILTERED", this.tableFilter);
          } else if (
            el.field === keys &&
            params[keys] === "desc" &&
            el.orderBy !== params[keys]
    ...
    updateData(index) {
      this.data = this.tableFilter[index];
    ...
    deleteData(index) {
      this.forDelete = [this.tableFilter[index]];
